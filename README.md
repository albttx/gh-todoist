...
| `pick` | Interactive multi-select over a repo's open issues (assigned to you when outside a repo). `/` filters, `↓` drops into the list, `enter` selects while filtered and pushes when not (the highlighted issue if none are ticked), `esc` backs out then quits. `--assign[=user]` filters by assignee. Already-tracked ones are hidden. |
